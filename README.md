...
| | | | |
|---|---|---|---|
| **157** 페이지 | **196** 커밋 | **129** 검수 지적 | **0** 404 유발 링크 |
| **9,213** CSS 줄 | **970** JS 줄 · 14파일 | **3,561/3,561** 이미지 alt | **180** 잔여 하드코딩 색상 |

> **수치 기준** — 2026-08-03 `main` 브랜치 실측값입니다. 데스크탑 전 페이지 생성·연결 완료 시점이며, 모바일 반응형은 미착수입니다. 집계 기준이 바뀐 항목은 §잔여 부채에 정정 이력을 남깁니다.
...
| 항목 | 07-29 | 07-30 | 07-31 | 08-01 | 08-02 | 08-03 |
|---|---|---|---|---|---|---|
| HTML | 1 | 80 | 125 | 136 | 152 | **157** |
| `style.css` | 1,546 | 2,044 | 4,306 | 6,524 | 8,379 | **9,037줄** |
| JavaScript | 0 | 93 | 93 | 139 | 788 | **970줄** (14파일) |
| 이미지 | 61 | 2,284 | 2,665 | 2,834 | 3,530 | **3,561** |
| 커밋 | 1 | 85 | 117 | 151 | 180 | **196** |
| 검수 지적 | — | 21 | 24 | 35 | 49 | — |

CSS 총계는 `style.css` 9,037 + `tokens.css` 176 = **9,213줄**, `!important` **0건**입니다.

## 페이지 구성 — 157

| 구획 | 수 | 내역 |
|---|---|---|
| 카테고리 | **67** | 대분류 목록 12 · 세부 39 · 얼리버드 8 · 이벤트 8 |
| 데스크(뉴스) | **33** | 허브 1 · 카테고리 17 · 전체 통합 15 |
| 크리에이터 | **13** | 홈 · 개설 문의 · 가이드 · 멘토 · 프로필 · 클래스 · 리뷰 · 질문 · 수익 · 정산 · 세금 · 스튜디오 · 예약 |
| 매거진 | **12** | 허브 2 · 카테고리 8 · 아티클 2 |
| 계정 (마이 둥지) | **9** | 대시보드 · 내 클래스 · 올나잇패스 · 찜한 둥지 · 주문내역 · 쿠폰 · 회원정보 · 알림 · 졸업모 |
| 강의 상세 (PDP) | **1** | 데모 1종 — 전 강의 카드 공통 연결 |
| 기타 | **22** | 메인 2 · 큐레이션 3 · 글로벌 할인 2 · 회사·서비스 · 고객센터·정책 · 사이트맵 |
| **합계** | **157** | |

**대분류 8계열** (괄호 = 세부 카테고리 수)

```
개발&IT 7 · 커리어&머니 6 · 디자인&크리에이티브 6 · 사진&영상 4
드로잉&일러스트 4 · 운동&마음챙김 4 · 공예&핸드메이드 4 · 요리&베이킹 4
                                                        합계 39
```

세부 개수 편차를 4~7로 통제해 메가 메뉴 패널 높이를 균일하게 유지했습니다.

**과금 모델을 분류 축과 교차**시켰습니다. 개발&IT·커리어&머니 = 단건(소장 가치가 높다) /
디자인&크리에이티브 = 구독+단건 / 나머지 5계열 = 구독(폭넓게 소비한다).
이 규칙이 메가 메뉴 배지 · 카드 태그 · 장바구니 · 계정 화면까지 관통합니다.
...
| 08-03 | 크리에이터 7종 · 알림 팝오버 · **링크 404 전량 제거** | **157** | — |
| | | | **누적 129건** |
...
뒷문장 때문에 `overflow: hidden` + `transform` 조합으로 구현됐고, `prefers-reduced-motion` 대응까지 함께 들어갔습니다.
...
매거진과 데스크(뉴스) 시스템을 추가해 80 → 125페이지로 확장했습니다.
...
| 이미지 라운딩 `desk.html` 누락 | "전체 적용" 후에도 `desk.html`만 빠짐 |
| 기사 상세 링크 미연결 | 링크 작업 완료 보고 후에도 `desk.html`에는 미부착 |
...
상태를 JS로 흉내내지 않고 **파일로 분리**했습니다. `index.html`(비로그인) ·
`index-member.html`(로그인 후) 2종이며, 차이는 헤더 유틸 영역과 개인화 섹션뿐입니다.
서버가 있어야 정해지는 상태는 상태별 파일로 명시한다는 원칙입니다.
...
| **결제방식 규칙 검증** | 개발&IT·커리어&머니 = 단건 / 디자인&크리에이티브 = 구독+단건 / 나머지 5계열 = 구독 전용. 내 클래스·위시리스트·주문내역이 규칙과 어긋나는지 전수 확인 |
...
크리에이터 측 관리 화면 **7종**을 추가해 **데스크탑 버전의 전 페이지 생성과 연결을 완료**했습니다.
...
크리에이터 홈의 빈 링크를 전부 연결해, **홈 → 각 관리 화면 동선이 끊기지 않도록** 마감했습니다.
...
사이트 어디서든 로고를 누르면 홈이 아니라 배포 루트로 빠지던 문제입니다.
첫 클릭이 로고인 방문자가 많다는 점에서 노출도가 높은 결함이었습니다.
...
**배지 동기화가 요점입니다.** 안 읽음이 0이 되면 숫자를 `0`으로 표시하는 게 아니라 배지를 숨깁니다.
`0`이 남아 있으면 스크린리더가 "0건 읽지 않음"을 계속 읽고,
시각적으로도 알림이 있는 것처럼 보입니다.
...
| 08-03 (1) | 165 | 로고 링크 157건 → `index.html` |
| 08-03 (2) | 9 | 알림 링크 156건 → 드롭다운으로 대체 |
| 08-03 (3) | 7 | 로그인·회원가입 → `<button>` 전환 |
| **08-03 (최종)** | **0** | 잔여 7건 개별 처리 |
...
`href="#"`로 두면 "눌러도 아무 일이 없는 링크"가 되지만,
버튼으로 바꾸면 **동작을 트리거하는 요소인데 아직 핸들러가 없다**는
정직한 마크업이 됩니다. 결제 진입점은 본래 링크가 아니라 버튼입니다.
...
**"이동이 아니면 링크로 쓰지 않는다"는 원칙이 사이트 전역에 일관되게** 적용됐습니다.
...
08-02의 카탈로그 대조 작업 연장선입니다.
숫자가 표시되는 자리마다 **실제 데이터와 일치하는지 확인**하는 절차가 이어졌습니다.
...
```
lang="ko" 157/157          ·  skip-link "본문 바로가기" 157/157
alt 서술형 2,556           ·  장식 이미지 alt="" 1,005
aria-label 3,846 · aria-hidden 4,784 · role 2,900 · tabindex 2,167
aria-selected 1,274 · aria-controls 1,571 · aria-pressed 964
aria-expanded 377 · aria-haspopup 345 · aria-current 270 · aria-disabled 154
prefers-reduced-motion 10블록
```

숨김 텍스트로 상태값을 함께 제공합니다.
별점 그래픽은 `alt=""`로 두고 옆의 `a11y-hidden` 텍스트가 `평점 5점 만점에`를 읽어줍니다.
장바구니·알림 뱃지에는 `2개 담김` · `2건 읽지 않음`.
정보를 이미지가 아니라 텍스트가 책임지는 구조입니다.

**남은 개선 과제** — 모달 포커스 가두기·Esc 닫기 실측 검증, `aria-live` 사용 0건
(필터·합계 변경의 음성 안내 미구현), Lighthouse·axe 자동 점검 미측정.

## 디자인 토큰

`tokens.css`(176줄)를 분리하고 컬러를 **원시 토큰 → 시맨틱 별칭** 2계층으로 구성했습니다.
...
| 항목 | 07-30 | 07-31 | 08-01 | 08-02 | 08-03 |
|---|---|---|---|---|---|
| `style.css` | 2,044줄 | 4,306줄 | 6,524줄 | 8,379줄 | **9,037줄** |
| 하드코딩 색상 (16진수) | 2건 | 21건 | 88건 | 121건 | **144건** ⚠️ |
| `!important` | **0건** | **0건** | **0건** | **0건** | **0건** |

이전 프로젝트(JW Anderson)에서는 시맨틱 별칭을 9개 선언해두고도 컴포넌트가 원시 토큰을
직접 호출해 **별칭 참조가 0회**였습니다. 이번에는 그 지점을 착수 전 요구사항으로 못 박았고,
**역할 이름 실참조 146회**로 계층이 실제로 작동합니다.

`z-index`는 실사용 8개 값(`-1 · 1 · 5 · 40 · 50 · 60 · 80 · 200`)이나
단계 고정 스케일이 없어 토큰화 대상으로 등재했습니다.
...
라이브러리 의존 0. jQuery·번들러 없이 순수 DOM API입니다.

```
search.js          136   헤더 검색 자동완성
class-filter.js    122   카테고리 필터 + 페이저
notifications.js   101   알림 팝오버
auth-modal.js       95   로그인 / 회원가입 모달
nav.js              93   메가 메뉴 (tablist)
myclass-tabs.js     93   내 클래스 탭 + 페이저
cart.js             71   담은 둥지 · 합계 재계산
cert.js             54   수료증 정렬 · 더보기
countdown.js        47   프로모션 마감 카운트다운
carousel.js         46   컬렉션 캐러셀
order-tabs.js       37   주문 내역 탭
cm-tabs.js          34   크리에이터 클래스 상태
mag-more.js         27   매거진 더보기
toggle.js           14   알림 설정 스위치
                   ───
                   970줄 / 14파일
```

모든 모듈이 즉시실행 함수 + `"use strict"`로 전역 오염을 차단하고,
대상 요소가 없으면 즉시 `return`합니다. 157페이지에 같은 스크립트를 실어도
해당 화면에서만 동작하는 구조입니다(조건 분기 대신 존재 확인).

**JS가 없어도 무너지지 않는 지점** — 메가 메뉴는 링크 자체가 동작하고,
검색 폼은 `action="categories.html"`로 서버 제출 경로가 살아 있으며,
필터 미동작 시 전체 목록이 그대로 보이는 것이 기본 상태입니다.
카운트다운은 목표 일시를 코드에 박지 않고 마크업 값을 읽어 시작합니다.
...
| 인바운드 링크 | **1,853건** |
| 연결된 페이지 | **83개** — 강의 카드 또는 랭킹이 존재하는 전 페이지 |
| 미연결 74개 | 뉴스 데스크 · 매거진 · 정책 · 계정 서브 · 크리에이터 |

## 1,853건의 내역

인바운드 링크는 두 경로에서 나옵니다.

| 경로 | 산식 | 건수 |
|---|---|---|
| 강의 카드 | 카드 **895** × 2 (썸네일 + 제목) | 1,790 |
| 랭킹 컴포넌트 | `rank-card__thumb` · `rank-mini__thumb` 썸네일 1건씩 | **63** |
| | | **1,853** |

```
categories.html         카드 152 → 링크 304
best.html               카드  24 → 링크  68   (카드 48 + 랭킹 20)
index.html              카드  33 → 링크  66
category-dev-it-1.html  카드  16 → 링크  32
```

`best.html`이 유일하게 카드×2 규칙을 벗어나는 것처럼 보이는 이유가 여기 있습니다.
훗업 차트 랭킹은 `course-card`가 아닌 자체 마크업(`rank-card` · `rank-mini`)을 쓰고,
썸네일 하나당 링크 1건입니다. **규칙 위반이 아니라 별도 컴포넌트**입니다.
...
| 항목 | 07-31 | 08-01 | 08-02 | 08-03 |
|---|---|---|---|---|
| 루트 절대경로 링크 | 3,389 | 573 | 193 | **0** ✅ |
| 하드코딩 색상 (16진수) | 21 | 88 | 121 | **144** ⚠️ |

## 집계 기준 정정 — 131 → 180

08-03 마감 시점에 **집계 스크립트를 하나로 고정하는 작업**을 수행했고,
그 결과 기존 공표 수치가 실제보다 작았다는 것이 드러났습니다.

| 구분 | 기존 공표 | 재측정 | 판정 |
|---|---|---|---|
| 16진수 (주석 제외) | 131 | **144** | 부채 — 주석 처리 기준 차이로 13건 누락 |
| ├ gradient 라인 | — | 53 | |
| └ 단색 | — | 91 | |
| 고유 색상 수 | 62 | **75** | |
| `rgb()` 알파 — 흑백 계열 | **집계 대상 아님** | ~200 | **부채 아님** — 투명도는 색상 토큰의 대상이 아님 |
| `rgb()` 알파 — 브랜드색 십진 표기 | **집계 대상 아님** | **36** | **부채** — 토큰 연동이 끊김 |
| **합계** | 131 | **180** | |

정정의 핵심은 두 번째 축입니다. 기존 집계는 **16진수만 셌고**,
`rgb(22 103 250 / 0.55)` 형태는 부채 계산에서 통째로 빠져 있었습니다.
이 값은 `--color-brand-base`(`#1667fa`)의 십진 표기로,
**토큰을 바꿔도 36곳이 따라오지 않습니다.**

반면 `rgb(255 255 255 / 0.25)` 같은 흑백 알파값은 그림자·오버레이용이며
사용처마다 투명도가 달라지므로 토큰화 대상이 아닙니다. 표기법 자체는
현대 공백 구문으로 통일돼 있어 규율은 유지되고 있습니다.

**해소 방향** — 브랜드 알파값은 `--color-brand-base-rgb: 22 103 250`을 신설해
`rgb(var(--color-brand-base-rgb) / 0.55)` 형태로 전환하면 토큰 연동이 복구됩니다.
16진수 144건은 반복 사용값부터 토큰으로 승격하되,
정책 페이지 상태색처럼 대응 토큰이 없는 경우는 상태색 토큰군 신설이 선행돼야 합니다.

## 왜 이 축만 밀렸는가

접근성 지표는 1 → 157페이지까지 한 번도 무너지지 않았습니다.
`alt`나 `h1`은 **페이지 생성 템플릿에 박혀 있어 자동으로 따라오지만**,
색상은 새 컴포넌트를 만들 때마다 **매번 판단해야 하는 항목**이기 때문입니다.

링크는 3,389 → 0으로 상환됐고 색상은 밀렸습니다.
두 축의 차이가 이 프로젝트에서 가장 많이 배운 지점입니다.
**규율은 의지가 아니라 구조로 유지됩니다.**
...
| 영역 | 상태 |
|---|---|
| 메인 랜딩 · 로그인 전/후 분리 (`index` / `index-member`) | ✅ |
| 카테고리 67 (대분류·세부·할인 필터) | ✅ |
| 큐레이션 · 글로벌 할인 | ✅ |
| 매거진 12 (허브·카테고리·아티클) | ✅ |
| 데스크 뉴스 33 (카테고리·페이지네이션·기사 상세) | ✅ |
...
| 태블릿 768–1279px | 🔸 축소 대응만 (33블록) |
| **모바일 ≤767px** | ⬜ **다음 단계** — 구간 표준화만 완료 |
| 하드코딩 색상 정리 | ⬜ 미착수 (180건) |
| Lighthouse · axe 자동 점검 | ⬜ 미측정 |
...
```css
/* 표준 구간 */
@media screen and (max-width: 767px)  { … }   40블록   ← 미착수
@media screen and (max-width: 1279px) { … }   33블록   🔸 축소 대응만
min-width: 1280px                              기본     ✅

/* 접근성 구간 */
@media (prefers-reduced-motion: reduce) { … }  10블록

/* 표준 외 — 점검 대상 */
@media (max-width: 720px) { … }   6블록
@media (max-width: 760px) { … }   1블록
```

미디어쿼리는 전역 3블록이 아니라 **컴포넌트 정의 직후에 배치**했습니다.
관련 규칙이 한자리에 모여 수정 지점을 찾기 쉬운 대신, 구간 전수 점검이 별도 절차로 필요합니다.
그 점검에서 **표준 외 쿼리 7건**이 나왔습니다. 본문 폭 제한(`article` 720px)에서 파생된
값으로 보이나 구간표에는 없습니다. 767 / 1279로 흡수할지 별도 구간으로 승격할지 판단 대기입니다.

157개 페이지에 헤더가 복제된 구조라, 반응형 적용은
**공통 컴포넌트를 한 번 정의하고 전 파일에 일괄 반영**하는 방식이 됩니다.
이 과정에서 하드코딩 색상 정리와 표준 외 구간 처리를 같은 회차에 묶는 것이 효율적입니다.
...
여기에 하나를 덧붙이게 됐습니다. **자기 집계도 검수 대상입니다.**
하드코딩 색상 131건은 오래 인용된 수치였지만, 집계 기준을 고정해 다시 재보니 180건이었습니다.
남의 보고를 대조하는 절차를 자기 문서에 적용하지 않으면 같은 종류의 오류가 남습니다.
...
Hoot-up_claude-code/
├── index.html                # 메인 — 비로그인
├── index-member.html         # 메인 — 로그인 후
├── categories.html           # 전체 클래스 152 · 헤더 검색 인덱스 원천
├── sitemap.html              # 전체 진입점
├── category-*.html           # 대분류 12 · 세부 39 · 얼리버드 8 · 이벤트 8
├── best/new/signature.html   # 큐레이션 3
├── earlybird-1 / event-1.html # 글로벌 할인 2
├── course-responsive-publishing.html  # PDP 데모 1종
├── desk*.html                # 뉴스 허브 1 · 카테고리 17 · 전체 통합 15
├── magazine*.html            # 매거진 허브 2 · 카테고리 8 · 아티클 2
├── account*.html             # 마이 둥지 9
├── creator*.html             # 크리에이터 13
├── cart.html                 # 담은 둥지
├── (회사 · 서비스 · 고객센터 · 정책)
├── css/
│   ├── tokens.css            # 디자인 토큰 176줄
│   └── style.css             # 컴포넌트 스타일 9,037줄
├── js/                       # 14모듈 970줄
└── assets/ · svg/            # brand · course · avatar · category · blog · news · banner
```
