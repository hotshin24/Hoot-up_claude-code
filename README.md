...
커밋 약 32건(11:05 ~ 23:47). 신규 페이지 2종으로 **159페이지**가 됐습니다.
...
커밋 약 21건(10:10 ~ 00:00). 신규 페이지 4종으로 **163페이지**가 됐습니다.
...
커밋 20건(13:10 ~ 22:46). 신규 페이지 1종으로 **164페이지**.
...
**접근성을 위해 넣은 것이 반응형을 무너뜨린 자리**입니다.
...
`contain` · `position: relative`만 먹혔는데, 이것이 **"절대요소가 탈출 중"** 이라는 결정적 신호였습니다.
...
범위는 **08-10 낮 10:50 ~ 08-12 자정**, 반응형 커밋 약 27건입니다.
...
행 간격(10px)이 하단 패딩(23px)보다 작아 `max-height`를 늘리는 방식으로는 2행이 새어 나옵니다.
...
| **링크를 두 번 눌러야 이동** | 미확인 | **이월** |

세 번째가 미해결입니다.
`@media (hover:hover)` 가드 189곳, `overflow-x` 값, `-webkit-overflow-scrolling`을 차례로 의심했으나
에뮬레이터에서 재현되지 않았습니다.

> "**세 번째만 되돌려줘.**" → "**반응형 다 끝나면 그때 다시 보자.**"

시도한 것 중 둘을 원복하고 이월했습니다.
**원인을 모르는 상태에서 변경을 쌓지 않는 판단**입니다.
08-10의 가로 스크롤 건처럼, 실기기에서만 나는 것은 실기기로 좁혀야 합니다.
...
- **푸터 적용 범위** — `section` → `div` 치환이 157페이지 × 4 = 628개입니다.
  당시 페이지는 159개이므로 2페이지가 빠져 있습니다.
  `course-watch` · `certificate`가 전체 화면 구성이라 푸터가 없는 것으로 보이나, 확인이 필요합니다.
...
- **캐시버스터 수동 관리** — 공용 스크립트를 고칠 때마다 참조 HTML 전체의 `?v=N`을 올려야 합니다.
  08-09에 41페이지를 일괄 처리했으나 빌드 도구가 없어 자동화 경로가 없고,
  08-11 변경분(`common.js` · `carousel.js` · CSS)은 **배포 전 일괄 상향이 필요합니다.**

### 08-11 신규 등재

- **[이월] 태블릿 · 모바일 링크 두 번 탭** — 링크를 두 번 눌러야 이동합니다.
  `@media (hover:hover)` 가드 189곳 · `overflow-x` 값 · `-webkit-overflow-scrolling`을
  차례로 의심했으나 에뮬레이터에서 재현되지 않아, 시도 중 둘을 원복하고 이월했습니다.
  반응형 마감 후 실기기로 재점검합니다.
...
**M8 반응형은 08-09 착수, 08-11까지 대부분의 페이지 유형을 마쳤습니다.**

```
데스크톱 우선 3단   1280 / 768 / 480
특수 구간           1060(차트 · 페스타) · 980(약관) · 870(베스트) · 375(초소형)
```

| 범위 | 상태 |
|---|---|
| 공용 (헤더 · 메가 메뉴 · 푸터) | ✅ 08-09 |
| `index.html` 전 섹션 | ✅ 08-09 |
| 카테고리 · 상세 · 브랜드 스토리 · 데스크 · 뉴스 · 매거진 | ✅ 08-11 |
| b2b · 사이트맵 · 베스트 · 페스타 · 로드맵 · 올나잇 패스 | ✅ 08-11 |
| 크리에이터 4종(개설 · 스튜디오 · 정산 · 멘토) · 공지 · FAQ · 약관 · 환불 | ✅ 08-11 |
| **계정 9종 · 장바구니 · 크리에이터 스튜디오 관리 · 강의 보기 · 수료증 · 계약서** | ⬜ 잔여 |
...
남은 것은 로그인 이후 화면 계열입니다.

그다음 두 가지가 이어집니다.

- **실기기 검증** — 이월된 "링크 두 번 탭" 문제. 에뮬레이터로 재현되지 않아
  실기기로 좁혀야 합니다. iOS 검색 인풋 · 중점 줄바꿈도 같은 경로로 확인합니다.
- **M9 실측** — Lighthouse · axe. 문서 3종에서 유일하게 미측정으로 비어 있는 칸입니다.

배포 전에는 `common.js` · `carousel.js` · CSS 변경분에 맞춰 `?v=N`을 일괄 올려야 합니다.
...
아래 수치는 08-03(157페이지) 기준이며, 08-05~06 작업분은 반응형 착수 전 일괄 재측정합니다.
...
| 항목 | 07-31 | 08-01 | 08-02 | 08-03 | 08-05 | 08-06 | 08-07 | 08-09 | 08-11 |
|---|---|---|---|---|---|---|---|---|---|
| HTML | 125 | 136 | 152 | 157 | 157 | 159 | 163 | 164 | **164** |
| `style.css` | 4,306 | 6,524 | 8,379 | 9,037 | — | — | — | — | 재측정 예정 |
| JavaScript | 93 | 139 | 788 | 970줄 (14파일) | 신규 5종 | 신규 7종 | 신규 다수 | 신규 다수 | 재측정 예정 |
| 이미지 | 2,665 | 2,834 | 3,530 | **3,561** | — | — | — | — | 재측정 예정 |
| 커밋 | 117 | 151 | 180 | 196 | 214 | 약 246 | 약 267 | 약 287 | **약 314** |
| 지적 건수 | 24 | 35 | 49 | — | 34 | 33 | 28 | 42 | **누계 319** |

07-29~30은 1 · 80페이지, `style.css` 1,546 · 2,044줄, 커밋 1 · 85건입니다.

줄 수 · 이미지 총계는 08-03 이후 재측정 전입니다.
커밋은 08-06부터 작업 리뷰 기준 근사값이라 누계도 근사값이며,
08-08 기록 없음 · 08-10 이른 오전분 미집계입니다. 아래로 확정할 수 있습니다.

```bash
git rev-list --count HEAD
git log --oneline --since="2026-08-10 00:00" --until="2026-08-12 23:59" | wc -l
```
