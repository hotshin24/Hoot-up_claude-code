...
- **가로 스크롤 가드가 임시 조치** — `html { overflow-x: hidden }` + `body { overflow-x: clip }`으로
  막아둔 상태입니다. 어느 요소가 넘치는지는 특정하지 않았고, 가드가 증상만 가리고 있습니다.
  NFR-RWD-02(전 구간 가로 스크롤 미발생)를 충족했다고 보기 어렵습니다.
  반응형 확장 중 원인을 특정해 가드를 걷어내는 것이 목표입니다.
- **반응형 미적용 163페이지** — 공용과 `index.html`만 완료입니다.
  나머지는 데스크탑 레이아웃이 그대로 축소되어 보입니다.
- **캐시버스터 수동 관리** — 공용 스크립트를 고칠 때마다 참조 HTML 전체의 `?v=N`을 올려야 합니다.
  08-09에 41페이지를 일괄 처리했으나, 빌드 도구가 없어 자동화 경로가 없습니다.

---

## 다음 단계 — 반응형 전 페이지 확장

**M8 반응형은 08-09에 착수했습니다.** 공용 레이아웃과 `index.html` 전 섹션이 완료 상태입니다.

```
데스크톱 우선 3단   1280 / 768 / 480      + 특수 1060(차트) · 375(초소형)
...
공용을 먼저 잡은 것이 이 순서의 핵심입니다.
164개 파일에 헤더가 복제된 구조라, 헤더 · 메가 메뉴 · 푸터를 한 번 정의하면 전 페이지에 반영됩니다.
남은 것은 **페이지 유형별 본문**입니다 — 카테고리 그리드 67p, 데스크 33p,
매거진 12p, 계정 9p, 크리에이터 18p, PDP · 장바구니 · 정책 계열.

같은 유형끼리 레이아웃이 동일하므로 유형당 한 번씩 잡으면 됩니다.
페이지 신설을 멈춘 것도 이 때문입니다 — 신규 페이지에는 새 컴포넌트가 딸려 오고,
그만큼 반응형을 다시 짜야 합니다.

이후 **M9 실측**(Lighthouse · axe)이 남습니다.
문서 3종에서 유일하게 미측정으로 비어 있는 칸입니다.
...
| 항목 | 07-30 | 07-31 | 08-01 | 08-02 | 08-03 | 08-05 | 08-06 | 08-07 | 08-09 |
|---|---|---|---|---|---|---|---|---|---|
| HTML | 80 | 125 | 136 | 152 | 157 | 157 | 159 | 163 | **164** |
| `style.css` | 2,044 | 4,306 | 6,524 | 8,379 | 9,037 | — | — | — | 재측정 예정 |
| JavaScript | 93 | 93 | 139 | 788 | 970줄 (14파일) | 신규 5종 | 신규 7종 | 신규 다수 | 신규 다수 · 재측정 예정 |
| 이미지 | 2,284 | 2,665 | 2,834 | 3,530 | **3,561** | — | — | — | 재측정 예정 |
| 커밋 | 85 | 117 | 151 | 180 | 196 | 214 | 약 246 | 약 267 | **약 287** |
| 지적 건수 | 21 | 24 | 35 | 49 | — | 34 | 33 | 28 | **누계 266** |

07-29는 1페이지 · `style.css` 1,546줄 · 이미지 61 · 커밋 1입니다.

줄 수 · 이미지 총계는 08-03 이후 재측정 전입니다.
08-06 · 08-07 커밋이 작업 리뷰 기준 근사값이라 누계도 근사값이며,
08-08은 작업 기록이 없어 반영하지 않았습니다. 아래로 확정할 수 있습니다.

```bash
git rev-list --count HEAD
git log --oneline --since="2026-08-09 00:00" --until="2026-08-09 23:59" | wc -l
```
...
| **반응형 — 나머지 163페이지** | ⬜ **다음 단계** — 페이지 유형별 진행 |
