...
PRD v2.1 · UI/UX 설명서 · 기능정의서 v1.5 세 문서로 관리하며,
...
| CSS 하드코딩 색상 (16진수) | 21 | 88 | 121 | **144** ⚠️ |
| 브랜드색 `rgb()` 표기 | — | — | — | **36** ⚠️ |
| 하드코딩 색상 합계 | — | — | — | **180** ⚠️ |

**하드코딩 색상 180건이 유일하게 남은 부채입니다.**
`style.css`가 2,044줄에서 9,037줄로 늘어나는 동안 16진수 기준으로 2 → 21 → 88 → 121 → 144로 증가했습니다.
08-01에 `--color-brand-dark`를 신설하고 08-02에 중복 토큰을 통합했지만,
새 페이지가 쌓는 속도를 따라잡지 못했습니다.

접근성 지표가 157페이지까지 무너지지 않은 반면 이 축만 밀린 이유는 분명합니다.
`alt`나 `h1`은 페이지 생성 템플릿에 박혀 자동으로 따라오지만,
색상은 새 컴포넌트마다 **매번 판단해야 하는 항목**이기 때문입니다.

### 해소 방향

브랜드 알파값 36건은 `--color-brand-base-rgb: 22 103 250`을 신설한 뒤
`rgb(var(--color-brand-base-rgb) / α)` 형태로 전환하면 토큰 연동이 복구됩니다.
현재는 토큰 값을 바꿔도 이 36곳이 따라오지 않습니다.
16진수 144건은 반복 사용값부터 토큰으로 승격하면 상당수가 흡수되고,
대응 토큰이 없는 정책 페이지 상태색은 토큰군 신설이 선행돼야 합니다.

### 그 외 등재 항목

- **`z-index` 단계 스케일** — 실사용 8개 값(-1 · 1 · 5 · 40 · 50 · 60 · 80 · 200), 고정 스케일 미수립
- **표준 외 미디어쿼리** — `720px` 6건 · `760px` 1건, 흡수/승격 판단 대기
- **간격 토큰 중복** — `--space-card-gap` = `--space-md` (20px), "값이 같으면 이름도 하나" 규칙 확장 예정
- **`aria-live`** — 사용 0건, 필터·합계 변경의 음성 안내 미구현
- **시맨틱 별칭 전환** — 실참조 146회 / 원시 직접 1,948회(7%), 컴포넌트 전환 진행 중
...
**로드 순서** — preconnect → Google Fonts Raleway(400·600·700·800·900) →
Spoqa Han Sans Neo(spoqa.github.io CDN) → `tokens.css` → `style.css` → `js/*.js`(`</body>` 직전).
