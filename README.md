...
크리에이터 측 관리 화면 6종을 추가해 **데스크탑 버전의 전 페이지 생성과 연결을 완료**했습니다.
...
**배지 동기화가 요점입니다.**
안 읽음이 0이 되면 숫자를 `0`으로 표시하는 게 아니라 배지를 숨깁니다.
...
CSS는 새 클래스를 만들지 않고 기존 선택자에
`appearance: none` · `border: 0` · `font-family: inherit` · `cursor: pointer`를
병합해 **시각적 변화 없이** 처리했습니다.
...
| CSS 하드코딩 색상 | 21 | 88 | 121 | **131** ⚠️ |

**하드코딩 색상 131건이 유일하게 남은 부채입니다.**
`style.css`가 2,044줄에서 9,037줄로 늘어나는 동안 2 → 21 → 88 → 121 → 131로 증가했습니다.
08-01에 `--color-brand-dark`를 신설하고 08-02에 중복 토큰을 통합했지만,
새 페이지가 쌓는 속도를 따라잡지 못했습니다.

접근성 지표는 157페이지까지 한 번도 무너지지 않은 반면 이 축만 밀린 이유는 분명합니다.
`alt`나 `h1`은 페이지 생성 템플릿에 박혀 있어 자동으로 따라오지만,
색상은 새 컴포넌트를 만들 때마다 **매번 판단해야 하는 항목**이기 때문입니다.

페이지 생성이 마무리된 지금이 정리 시점입니다.
...
이 과정에서 하드코딩 색상 정리를 함께 처리하는 것이 효율적입니다.
...
| 하드코딩 색상값 | 2건 | 21건 | 88건 | 121건 | **131건** ⚠️ |
...
| JavaScript | 0 | 93 | 93 | 139 | 788 | **923줄** (13파일) |
| 이미지 | 61 | 2,284 | 2,665 | 2,834 | 3,530 | **3,561** |
| 커밋 | 1 | 85 | 117 | 151 | 180 | **194** |
...
크리에이터      13    홈 · 프로필 · 클래스 관리 · 리뷰 · 질문 · 정산 · 세금 · 스튜디오 예약 외
큐레이션         3    베스트 · 신규 · 시그니처
