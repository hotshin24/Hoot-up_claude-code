...
커밋 약 24건. 계정 9종과 크리에이터 스튜디오 9종을 훑어 **반응형 페이지 전개를 마쳤습니다.**

08-09 공용 → 08-11 공개 페이지 → 이번 로그인 이후 화면 순서였습니다.
개인화 영역은 표 · 목록 · 폼이 밀집해 있어 좁은 폭에서 가장 잘 무너지는 구간입니다.
...
**더블탭은 로컬 개발 서버(HTTP · LAN · 매 탭 무캐시 풀 리로드)에서만 나던 환경 아티팩트였습니다.**
...
템플릿에 박히지 않고 매번 판단해야 하는 항목은 계속 새어 나옵니다.
...
- **[배포 전 필수] 캐시버스터 `?v=1` 고정** — `style.css` · `common.js` · `tokens.css`가
  ...
  **GitHub Pages 배포본은 방문자 브라우저가 캐시된 옛 파일을 계속 씁니다.**
  참조 HTML 전체와 `import.css` 내부 `@import 'style.css?v='`까지 함께 올려야 합니다.
  08-09에 41페이지를 일괄 처리한 전례가 있으나 빌드 도구가 없어 자동화 경로가 없습니다.
...
- **문서형 3종 반응형 미확인** — `course-watch` · `certificate` · `creator-contract`가
  08-09~13 작업 기록에 등장하지 않습니다. 사이트 크롬 없이 문서만 있는 화면이라
  공용 헤더 · 푸터 반응형의 적용을 받지 않습니다. 별도 확인이 필요합니다.
- **`ul` 일괄 변환 후 태그 균형 검증** — 컴파운드 클래스(`cm-row cm-row--last`) 변환에서
  여는 태그 매칭이 누락돼 여닫기 불균형이 발생한 전례가 있습니다.
  스크립트 변환분은 W3C 검사로 균형을 확인해야 합니다.
...
- **캐시버스터 일괄 상향** — `style.css` · `common.js` · `tokens.css` · 신규 JS가
  `?v=1`에 고정된 채 내용만 바뀌었습니다. 참조 HTML 전체와
  `import.css` 내부 `@import 'style.css?v='`를 함께 올려야 방문자가 새 파일을 받습니다.
...
| `style.css` | 6,524 | 8,379 | 9,037 | — | — | — | — | — | 재측정 예정 |
| JavaScript | 139 | 788 | 970줄 (14파일) | 신규 5종 | 신규 7종 | 신규 다수 | 신규 다수 | — | 신규 3종 · 재측정 예정 |
| 이미지 | 2,834 | 3,530 | **3,561** | — | — | — | — | — | 재측정 예정 |
| 커밋 | 151 | 180 | 196 | 214 | 약 246 | 약 267 | 약 287 | 약 314 | **약 338** |
| 지적 건수 | 35 | 49 | — | 34 | 33 | 28 | 42 | 43+10 | **누계 359** |

07-29~31은 1 · 80 · 125페이지, `style.css` 1,546 · 2,044 · 4,306줄, 커밋 1 · 85 · 117건입니다.
...
git log --oneline --since="2026-08-12 00:00" --until="2026-08-13 23:59" | wc -l
```
