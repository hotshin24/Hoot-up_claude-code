...
세 번째 지시가 요점입니다. **작업 범위 밖에 남은 값 하나까지 추적**해 토큰을 신설하고 흡수했습니다.
...
**카테고리별 결제방식이라는 비즈니스 규칙**을 정의하고 여러 페이지에 교차 대조하게 했습니다.
...
토큰으로 흡수했습니다. 08-03에 등재해둔 해소 방향과 같은 판단입니다.
...
정작 치환은 16진수에만 적용했습니다. 브랜드색 `rgb()`는 오히려 73 → 78로 늘었습니다.
...
08-06 자기 검토에서 등재한 `role="tablist"` 미연결 건을 이날 처리했습니다.
...
혼재돼 있던 **6종을 3단으로 병합**하고 `style.css` 말미에 모았습니다.
...
07-31의 명시도 진단처럼, 값을 바꾸는 것으로는 해결되지 않는 유형입니다.
...
가드 CSS를 그대로 두면 측정 자체가 왜곡됩니다. 그 선언을 지운 상태에서 측정해야 넘침 지점이 좁혀집니다.
...
두 건 다 미리보기(Chromium)에서는 정상으로 보였습니다.
...
`flex-wrap: nowrap`으로 해결했습니다. 미리보기에서는 정상 1열로 보이던 자리입니다.
...
붙여준 스니펫의 출처 파일을 먼저 확인하고 착수하면 나오지 않는 항목입니다.
...
## 2026-08-15 — 코드 품질 · 접근성 · 성능 정리 회차

반응형이 끝난 뒤 이틀을 비우고, 화면을 늘리지 않는 작업만 한 회차입니다.
유효성 · 포맷 · 중복 · 토큰 · 접근성 · 성능을 순서대로 훑었습니다.
커밋 15건, 17:06 \~ 자정.

앞의 회차들과 성격이 다릅니다. 지금까지는 AI가 만든 것을 지적해 고치는 흐름이었는데,
이날은 **이미 통과한 코드를 다시 검사하는 흐름**이었습니다.
지적할 결함이 화면에 없으니, W3C Nu Validator · Lighthouse · axe를 돌리게 하고
그 결과 중 무엇이 실제 문제인지를 판정하는 것이 제 몫이었습니다.

### W3C 유효성 — 오류와 경고

| 시각 | 내용 |
|---|---|
| 17:06 | `category-dev-it-earlybird` · `-event` · `earlybird-1` — 필터바 뒤 잉여 `</div>` + 푸터 `site-footer__inner` 닫는 태그 누락. 각 3 errors → 0 |
| 17:27 | Section/Article lacks heading — `faq`의 `hfaq__list-sec`에 숨김 `<h2>`, `privacy` · `terms`의 `terms__body`를 `<section>` → `<div>`로 바꾸고 `terms__article`에 숨김 제목 |

한 페이지에서 발견한 `</div>` 오류를 형제 페이지 전체로 스캔해 세 곳을 찾았습니다.
07-31 이후 반복된 패턴입니다 — 한 건을 고치기 전에 같은 구조가 어디까지 복제됐는지 먼저 셉니다.

약관 계열 4종(`privacy` · `terms` · `refund` · `youth`)은 이 과정에서 구조가 통일됐습니다.
`refund` · `youth`가 이미 올바른 형태였고, 나머지 둘을 거기 맞춘 것입니다.

`account-notifications`의 `<input type="time">` 경고는 수용하지 않았습니다.
Nu Validator가 "not supported in all browsers"라고 알리지만 타깃 브라우저는 전부 지원합니다.
오류가 아니라 구식 조언이라 판단해 `type="time"`을 유지했습니다.

### HTML 포맷 통일

편집기 자동 포맷이 파일마다 다르게 적용돼 미커밋 84개가 쌓여 있었습니다.
Prettier로 밀면 doctype 대소문자 · void 태그 표기 · `<head>` 들여쓰기가 전부 바뀌므로 쓰지 않고,
편집기가 쓰던 js-beautify 설정을 그대로 재현했습니다.

```
2-space · indent_inner_html: false · end_with_newline: false
```

이미 포맷된 83개 파일에 멱등인지 먼저 확인한 뒤 164개 전체에 적용했습니다.
156파일 변경 중 실 수정은 3개이고 나머지는 순수 포맷입니다.
`.jsbeautifyrc`를 저장소에 넣어 다음부터 설정이 갈라지지 않게 했습니다.

### CSS 정리 4건

| 시각 | 내용 | 규모 |
|---|---|---|
| 18:37 | 선언블록이 같고 셀렉터만 다른 규칙을 콤마 셀렉터로 통합 | 272그룹 · 중복 654개 제거 |
| 18:47 | 캐시버스터 `?v=` 전면 제거 (HTML 164 + `import.css` 내부 `@import`) | 370건 |
| 19:37 | HTML 배너 구분선 주석 슬림화 — 3줄 `=====` 227 + 1줄 136 → `<!-- 라벨 -->` | 363개 · HTML 454줄 감소 |
| 19:50 | `@media (prefers-reduced-motion)` 11블록 → 전역 1블록 | `*, ::before, ::after` |

캐시버스터는 08-13 문서에 "`?v=1`에 고정된 채 내용만 바뀌었다"고 미해결로 남겨둔 항목입니다.
버전을 올리는 방식 대신 제거하고 `serve.py`의 no-store와 GitHub Pages 기본 캐시에 맡겼습니다.
수동 관리 항목을 없애는 쪽을 택한 것입니다.

주석은 "왜 그렇게 했는지"를 적은 것만 남기고 구획 표시용 배너만 줄였습니다.
자명한 설명은 지웠고, 판단 근거가 적힌 주석은 손대지 않았습니다.

`prefers-reduced-motion`은 애니메이션을 추가할 때마다 옆에 붙이던 방식이라 11개로 흩어져 있었습니다.
전역 한 블록으로 합치면서 커버리지가 완전해졌고, 이 블록에서 `!important` 4건이 생겼습니다.
전역 모션 차단은 컴포넌트 선언을 이겨야 하므로 `!important`가 정석입니다.
이 프로젝트에서 유일하게 허용한 자리이고, 다른 곳의 0건은 그대로입니다.

**대량 변환은 전부 렌더 해시로 검증했습니다.** 3개 페이지 × 데스크 · 태블릿 · 모바일에서
계산된 스타일을 뽑아 해시를 비교했고, 변환 전후가 동일한 것을 확인한 뒤 커밋했습니다.

### 토큰화 2건

| 시각 | 내용 |
|---|---|
| 20:12 | 리터럴 → 토큰 68곳. 타이포 58(`font-size` 17 + `line-height` 41) + 색상 10 |
| 20:21 | 색상 원시 토큰 → 시맨틱 별칭 957곳. `color→text-*` 521 / `background→surface-*` 250 / `border→border-subtle` 186 |

두 건 다 **값이 같다는 이유만으로는 바꾸지 않았습니다.**
타이포는 크기와 행간이 같은 스케일 단계에서 짝을 이룰 때만 치환했고,
색상은 속성 문맥으로 매핑해 `border` 토큰이 배경에 붙는 일이 없게 했습니다.
`brand-base`처럼 실제 브랜드색이거나 별칭이 없는 원시 토큰은 남겼습니다.

첫 스캔이 두 번 다 틀렸습니다. 색상은 6자리 hex만 세어 3건이라 했는데 축약형을 넣으면 11건이었고,
타이포는 `clamp()` 내부 px까지 세어 40건이라 했는데 유효한 것은 17건이었습니다.
**스캔 범위를 먼저 확인하지 않으면 과소와 과대가 한 번씩 나옵니다.**

### 접근성 측정과 개선

측정을 먼저 하고 개선 순서를 그 결과로 정했습니다.

```
Lighthouse (로컬 · index)   접근성 97 · 권장사항 100 · SEO 100 · 성능 64
```

이 성능 64는 로컬 서버 기준입니다. 뒤에서 배포본으로 다시 재면 90이 나옵니다.

**axe가 잡아낸 대비 위반 중 약 60%는 실제로는 문제가 아니었습니다.**
어두운 배경 위에 흰 글씨를 올린 곳인데, axe가 배경을 흰색으로 읽어 대비가 부족하다고 판정한 것입니다.
푸터(`#0f172a`)가 그런 경우입니다.
숫자를 그대로 받았다면 멀쩡한 브랜드색을 바꿀 뻔했습니다.
한 건씩 실제 배경색을 확인해 진짜 위반과 잘못 잡힌 것을 나눈 뒤, 세 건만 처리했습니다.

| 개선 | 내용 |
|---|---|
| 색상 대비 | `--color-text-muted` `#93a2b7`(2.59:1) → `#667286`(4.87:1). 작은 텍스트 WCAG AA 통과. 장식용 밝은 회색은 유지하고 다크 배경 회귀 0 확인 |
| 랜드마크 · 헤딩 | `aside.promo` → `section`, `.hero`와 `.countdown`의 중복 라벨 정리, `categories` 그리드에 숨김 `<h2>`. axe 랜드마크 · heading-order 0 |
| `aria-live` | `common.js`가 전역 `#a11y-live`(polite)와 `window.announce()`를 주입. HTML 무변경 |

`aria-live`는 페이지마다 속성을 붙이는 대신 전역 알림 영역 하나를 만들고
필요한 모듈이 `window.announce()`를 호출하는 구조로 했습니다.
현재 호출 지점은 14곳입니다 — 헤더 검색 · 알림 읽음 처리 · 강의 필터 · 더보기 · 페이저 ·
크리에이터 질문/리뷰의 답변 등록입니다.

```
class-filter.js       필터 · 더보기 · 페이저     3
creator-questions.js  필터 · 답변 등록 · 페이저   3
creator-reviews.js    필터 · 답글 등록 · 페이저   3
common.js             검색 · 알림 읽음          4
mag-more.js           더보기                  1
```

카운트다운처럼 초 단위로 갱신되는 값은 배선하지 않았습니다. 낭독이 끊이지 않기 때문입니다.

### 배포본 측정 — 로컬 도구가 놓친 것

여기까지 마치고 GitHub Pages에 반영한 뒤, 배포된 사이트에 Lighthouse를 다시 돌렸습니다.

```
성능 90 · 접근성 97 · 권장사항 100 · 검색엔진 최적화 100   (모바일 · 22:04)
```

접근성 미통과는 대비 한 종류 45건뿐이었는데, 그 내역이 문제였습니다.

| 컴포넌트 | 대비 | 전경 / 배경 | 건수 |
|---|---|---|---|
| `course-card__badge` | 4.30 | `#ffffff` / `#ed2040` | 27 |
| `course-card__rate` | 4.30 | `#ed2040` / `#ffffff` | 6 |
| 푸터 · 문의 등 다크 영역 | 3.67 | `#667286` / `#0f172a` | 11 |
| `blog-card__category` | 1.33 | `#49fca6` / `#ffffff` | 1 |

**세 번째 줄 11건은 이날 제가 만든 것입니다.**
`--color-text-muted`를 `#93a2b7`에서 `#667286`으로 어둡게 바꿔 밝은 배경 대비를 4.87로 올렸는데,
어두운 배경에서는 같은 변경이 6.88에서 3.67로 대비를 떨어뜨렸습니다.

그런데 axe로 확인했을 때는 이 회귀가 잡히지 않았습니다.
같은 날 앞에서 확인한 그 오판 — **푸터 배경을 흰색으로 읽는 성질** 때문입니다.
흰 배경 기준으로 계산하니 `#667286`이 통과로 나왔습니다.

```
axe 배경 오판의 두 얼굴
  앞에서   없는 위반을 만들어냈다 (60%가 실제로는 문제 아님)
  여기서   있는 회귀를 가렸다     (다크 11건을 통과로 판정)
```

노이즈만 만드는 줄 알았던 성질이 실제 결함을 숨기고 있었습니다.
로컬 도구 하나로는 검증이 성립하지 않고, **배포본 측정과 교차해야** 한다는 것이 이 건의 결론입니다.

### 대비 완성 — 97에서 100으로

| 시각 | 대상 | 조치 |
|---|---|---|
| 23:10 | 다크 영역 muted 텍스트 | 푸터 · 문의 · 구독 · 정산 · 로드맵 · 히어로 등 어두운 컨테이너 안에서 토큰을 밝은 값으로 다시 정의 |
| 23:26 | 할인 배지 · 할인율 | `danger-base`(`#ed2040`, 4.30) → `danger-dark`(`#cb102d`, 5.75). 실패 34 → 1 |
| 23:37 | 블로그 카테고리 라벨 | 민트 `brand-accent`(`#49fca6`, 1.33) → `brand-base`(`#1667fa`, 4.82). 실패 1 → 0 |

세 건 다 **전역 토큰을 바꾸지 않고 스코프를 좁혀** 처리했습니다.
`muted`도 `danger`도 밝은 배경과 어두운 배경에서 필요한 방향이 반대라,
전역 값을 하나로 정하면 한쪽을 고칠 때마다 반대쪽 대비가 떨어집니다.
배지는 항상 밝은 카드 위에만 놓이므로 컴포넌트 범위로 한정했고,
`muted`는 어두운 컨테이너 안에서만 다시 정의했습니다.

재측정 결과 **접근성 100**입니다.

### 이미지 최적화 — 성능 89에서 93으로

| 시각 | 내용 |
|---|---|
| 23:50 | 표시 크기보다 과하게 큰 JPG 19개를 최대 1000px · 품질 80으로 재압축. 약 8.2MB 절감. 재압축 후 오히려 커진 3개는 되돌림 |
| 00:02 | `assets`의 png · jpg **328개를 WebP로 전환**(품질 82). HTML 참조 2,396곳과 CSS 15곳 교체, 깨진 참조 0 확인 후 원본 삭제 |

`assets`가 12MB에서 **5.3MB**로, index 이미지가 1,871KB에서 **774KB**로 줄었습니다.
SVG 46개는 그대로 두었습니다. 벡터라 변환 이득이 없습니다.
폴백 없이 WebP만 쓰기로 한 것은 타깃 브라우저가 전부 지원하기 때문입니다.

**작업 도중 세 번 헛짚었습니다.**

`du`로 잰 `assets` 용량이 660MB로 나왔습니다. 외장 디스크의 클러스터 크기 때문에 부풀려진 값이었고,
실제 바이트로 다시 세니 20MB였습니다. 이미지 대부분은 이미 작았고(강의 썸네일 18KB),
3MB짜리 `night-sky.jpg`는 어디서도 참조되지 않는 파일이었습니다.

macOS 내장 `sips`로 WebP를 만들려다 실패했습니다. 이 도구는 WebP 쓰기를 지원하지 않아
`sharp`를 따로 설치해 처리했고, 설치 흔적은 커밋 전에 지웠습니다.

확장자가 `.jpg`에서 `.webp`로 한 글자 길어지면서 일부 줄이 줄바꿈 기준을 넘겨
포맷이 어긋났습니다. 이날 만들어둔 `.jsbeautifyrc`로 다시 정규화했습니다.

### 최종 측정

```
접근성 100 · 권장사항 100 · 검색엔진 최적화 100 · 성능 93   (배포본 · 모바일)
```

### 남은 것

성능 93에서 더 올리려면 이미지가 아니라 폰트입니다.
Google Raleway와 Spoqa가 렌더링을 약 1,590ms 막고 있어, 이것이 남은 항목 중 가장 큽니다.
`<img>`에 `width`/`height`를 넣는 것과 CSS · JS 압축이 그다음입니다.
알파 채널 색상 토큰과 시맨틱 별칭 나머지 전환은 이전 회차에서 넘어온 항목입니다.
...
- ~~**`aria-live`** — 08-13 실측 0건~~ → **08-15 종결.** `common.js`가 전역 `#a11y-live`와
  `window.announce()`를 주입하고 14개 지점에 배선했습니다
- **시맨틱 별칭 전환** — 08-13까지 실참조 163회 / 원시 직접 2,167회, 비율 **7.0%로 08-03과 동일**이었습니다.
  08-15에 속성 문맥 매핑으로 957곳을 전환해 **57.6%**(1,110 / 1,928)가 됐습니다.
  다만 린트가 아니라 일괄 정리라 새 컴포넌트가 다시 원시 토큰을 부르면 같은 자리로 돌아옵니다
- ~~**자동 접근성 점검** — Lighthouse · axe 미측정~~ → **08-15 종결.**
  배포본 모바일 기준 접근성 100 · 권장사항 100 · SEO 100 · 성능 93
...
- ~~**[배포 전 필수] 캐시버스터 `?v=1` 고정**~~ — **08-15 종결.** 상향이 아니라 370건 전면 제거로 처리하고
  `serve.py`의 no-store와 GitHub Pages 기본 캐시에 맡겼습니다. 아래는 당시 기록입니다.
  `style.css` · `common.js` · `tokens.css`가
...
- **검증 환경 한계** — 개발 환경의 미리보기 창은 폭을 768px 이하로 줄여도
  페이지에 1560px 폭인 것처럼 알립니다. 화면은 좁아 보이는데 CSS가 받는 값은 데스크탑이라
  모바일 실측이 불가합니다. 구조는 `getComputedStyle` · `scrollWidth` · DOM 상태로 검증하되,
  패딩 침범이나 터치처럼 눈으로 봐야 아는 항목은 배포본과 폰에서 따로 확인해야 합니다.
  08-13의 `rev-summary__stats` iOS 3열 건이 실례입니다 — 미리보기에서는 정상 1열이었습니다.
...
- **하드코딩 색상 잔여 61건** (08-13 기준 · 08-15에 26건) — 16진수 6 · 브랜드색 `rgb()` 55.
  08-05에 종결로 기록했으나 08-13 재측정에서 미해결로 확인된 항목입니다.
  브랜드색 `rgb(22 103 250 / α)` 계열이 29건으로 가장 많고,
  `--color-brand-*`에 알파를 실을 수 있는 표기로 정리해야 근본 해소됩니다.
- ~~**캐시버스터 일괄 상향**~~ — 08-15에 제거로 종결. `import.css` 164건 · `common.js` 161건이 `?v=1` 고정이었습니다.
...
- ~~**M9 실측** — Lighthouse · axe~~ — 08-15 종결. 배포본 기준 접근성 100 · 권장사항 100 · SEO 100 · 성능 93.
  성능은 폰트 렌더 차단(약 1,590ms)이 남은 최대 항목입니다.
...
| `!important` | 0 | 0 | 0 | 0 | 0 | **0** (08-15 전역 reduced-motion 4건) |
...
`prefers-reduced-motion`은 08-15에 11블록에서 전역 1블록으로 통합됐습니다. `lang="ko"` 164/164.
`aria-live`는 08-13 시점 0건이었고, 08-15에 전역 알림 영역 방식으로 구현했습니다(호출 14곳).
...
| `!important` | **0건** | **0건** | **0건** | **0건** | **0건** | **0건** | **0건** (08-15 전역 reduced-motion 4건) |
...
## JavaScript — 48모듈 3,705줄 (08-13 기준 · 08-15에 3,738줄)

파일 하나가 화면 기능 하나를 맡습니다. 아래 표는 08-13 재측정 기준이며,
08-03 시점 970줄 / 14파일에서 **3.8배**가 됐습니다.
08-15에 `common.js`가 654 → 677줄로 늘어(전역 `aria-live` 주입) 합계는 3,738줄이 됐습니다.
...
| 항목 | 08-01 | 08-02 | 08-03 | 08-05 | 08-06 | 08-07 | 08-09 | 08-11 | 08-13 | 08-15 |
|---|---|---|---|---|---|---|---|---|---|---|
| HTML | 136 | 152 | 157 | 157 | 159 | 163 | 164 | 164 | 164 | **164** |
| `style.css` | 6,523 | 8,155 | 9,037 | 9,098 | 8,994 | 9,175 | 9,654 | 9,872 | 10,174 | **9,883** |
| 선언 블록 | — | — | 3,511 | — | — | — | — | — | 3,930 | **3,254** |
| `tokens.css` | — | — | 176 | — | — | — | — | — | 189 | **189** |
| JavaScript | 139 | 788 | 970 (14파일) | 신규 5종 | 신규 7종 | 신규 다수 | 신규 다수 | — | 3,705 (48파일) | **3,738 (48파일)** |
| 이미지 | 2,834 | 3,530 | 3,561 | — | — | — | — | — | 3,765 | **3,765** |
| `assets` 용량 | — | — | — | — | — | — | — | — | 12MB | **5.3MB** |
| 커밋 (일일) | 34 | 26 | 19 | 18 | 33 | 21 | 22 | 19 | 19 | **15** |
| 커밋 (누계) | 152 | 178 | 197 | 219 | 252 | 273 | 297 | 324 | 353 | **377**  |
...
08-13 마감 시점 누계는 353이고, 08-14 README 정정 1건과 08-15 문서·정리 회차 23건이 더해져 현재 377입니다.
...
| 하드코딩 색상 정리 | 🔸 205건 → 61건(08-13) → **26건**(08-15 · 16진수 4 · 브랜드 `rgb()` 22) |
...
| 배포 전 캐시버스터 처리 | ✅ 08-15 전면 제거(370건) |
...
잘못 작성된 코드를 찾는 일은 어렵지 않았습니다. 화면이나 코드에 그대로 드러나기 때문입니다.
찾기 까다로웠던 것은 일곱 가지였습니다.

**1. 화면에 드러나지 않는 결함**

화면이 멀쩡해서 리뷰로는 잡히지 않는 것들이 있었습니다.
W3C validator를 돌려야 마크업 오류가 검출되고, 마크업을 직접 열어야 의미 없는 DOM 중첩이 확인되며,
원본 기획서와 문구를 맞춰봐야 텍스트가 어긋난 것이 나옵니다.
...
07-31에 세 번 반복됐습니다. 전체를 지시해도 실제 적용은 일부에서 멈추는데,
반영된 영역만 확인하면 통과해버립니다. **적용됐어야 할 범위를 따로 짚어 확인해야** 합니다.

**4. 페이지가 늘어나는 만큼 규칙 위반이 누적되는 경우**
...
한 번 정해두면 자동 반영되는 자리에 넣은 항목만 끝까지 유지됐습니다.
...
매번 판단해야 하는 항목은 일괄 정리로 유지되지 않습니다.
템플릿에 넣거나 커밋 전에 검사하는 린트 규칙을 두지 않는 한,
페이지가 늘어나는 만큼 다시 누적됩니다.

마감 시점에는 그런 규칙을 만들지 못했습니다.
이틀 뒤 별도 회차에서 속성 문맥 매핑으로 957곳을 전환했고, 비율은 7.2%에서 57.6%가 됐습니다.
색상도 61건에서 26건으로 줄었습니다.

그래도 08-05와 같은 방식이었습니다.
`color`는 `text-*`로, `background`는 `surface-*`로 바꾸는 규칙을 사람이 정해
이미 쓰여 있는 것을 한 번에 손봤을 뿐, 다음에 만들 컴포넌트를 검사하는 규칙은 여전히 없었습니다.
새 페이지를 만들기 시작하면 같은 상태로 돌아올 가능성이 큽니다.

두 번 다 그 시점의 코드는 정리됐지만, 다시 늘어나는 것을 막을 방법은 만들지 못했습니다.
한 번에 손보면 되는 것과 규칙을 세워야만 되는 것의 차이를 확인한 것이 이 항목의 결론입니다.
...
기준을 두 축으로 넓혀놓고 정작 검산은 옛 기준 한 축으로 한 것이 원인입니다.
커밋 수(약 338 → 실측 353)와 `style.css` 줄 수(9,037 → 10,174)도 같은 성격으로 어긋나 있었습니다.
08-15의 첫 스캔도 마찬가지였습니다 — 색상은 6자리 hex만 세어 3건이라 했는데 실제는 11건,
타이포는 `clamp()` 내부 px까지 세어 40건이라 했는데 유효한 것은 17건이었습니다.

세 번을 세고서야 맞은 셈입니다.

세 번째 집계는 08-13에 검증만 하는 세션을 따로 잡아 처리했습니다.
...
08-15에 axe로 대비를 점검했을 때, 잡힌 위반의 약 60%는 실제로는 문제가 아니었습니다.
어두운 배경 위의 흰 글씨를 흰 배경 위에 있는 것으로 읽었기 때문입니다.
그런데 같은 성질이 반대로도 작동했습니다.
`--color-text-muted`를 어둡게 바꿔 밝은 배경 대비를 올린 변경이
어두운 배경에서는 대비를 6.88에서 3.67로 떨어뜨렸는데, axe는 이것을 통과로 판정했습니다.
배포본에 Lighthouse를 돌리고 나서야 11건이 드러났습니다.

미리보기 창도 같습니다. 폭을 768px 이하로 줄여도 페이지에는 1560px로 알리므로,
그 화면으로 확인한 반응형 결과는 근거가 되지 못합니다.
도구가 어디까지 맞는지 모르면 그 도구로 확인한 결과도 검증된 값이 아닙니다.

---

앞의 여섯 가지가 AI 산출물과 자기 집계를 대조하는 얘기라면, 일곱 번째는 그 대조에 쓰는 수단 자체입니다.
검수는 화면을 보는 일이 아니라 **원본·표준·개수와 대조하고, 그 대조 수단까지 확인하는 일**이라는 것이
지금까지의 결론입니다.
...
│   ├── tokens.css           # 디자인 토큰
│   └── style.css            # 컴포넌트 스타일 (9,883줄 · 색상 리터럴 26건 잔여)
├── js/                      # common.js 외 모듈
