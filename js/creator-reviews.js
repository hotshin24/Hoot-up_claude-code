/* 크리에이터 수강생 리뷰 (creator-reviews.html) — 데모
   · 필터(전체 / 답글 미작성) + 5개씩 페이지네이션 (.pager 재사용 — 카테고리/블로그와 동일)
   · '답글 미작성' = 아직 내 답글이 없는 리뷰(.rv-answer 존재)
   · 필터 변경 시 1페이지로 초기화, 페이저는 필터된 결과 기준으로 재계산
...
(function () {
  ...
  var page = 1;
  var filter = 'all';   // 'all' | 'noreply'

  function items() { return [].slice.call(list.querySelectorAll('.rv-item')); }

  // 필터 조건: 'noreply' 는 아직 답글이 없는(=답글 입력창이 있는) 리뷰만
  function matches(el) {
    if (filter === 'noreply') return !!el.querySelector('.rv-answer');
    return true;
  }
  ...
  // 답글 등록 — .rv-answer(입력창) → .rv-reply(내 답글) 로 전환
